...
class handle_null_values(object):
    ...
    def delete_var_with_null_more_than(self, dataframe=None, percentage=30):
        ...
        describe_dataframe = dataframe.select([count(when(
            col(c).isNull() | col(c).contains("NULL") | col(c).contains("null") | col(c).contains("None") | col(
                c).contains("NONE") | col(c).contains("none"), c)).alias(c) for c in dataframe.columns]).toPandas()
